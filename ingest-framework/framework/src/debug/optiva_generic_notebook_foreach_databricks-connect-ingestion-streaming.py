...
from sparkbuilder.dq.dq import Dq
...
class Args:
    def __init__(self, config_path, data_product_name,table_name,source_data_type, source_table_type, source_reader_options, audit_write, verbose, run_dq_rules,source_filepath,pkeys, streaming, trigger, writes, transformations, source_orderBy_column, dq_config_table, source_extraction_type, source_workload_type):
        ...
        #TODO: Add to the final notebook
...
def get_user_args():
    ...
    #TODO: ADD to the logic
    source_extraction_type = "IE" # FE for Full Extract or IE for Incremental Extract
    ...
    return Args(config_path=None,
            ...
            source_workload_type=source_workload_type,
            source_extraction_type=source_extraction_type
        )
...
def main():
    ...
    pipeline_config = {
        ...
        # TODO: add  column to the config table
    ...
    #Data Quality
    # Debug
    df = spark.read.table("dev.tempjobs.optiva_test_ingestion_history")

    dq = None
    if config["run_dq_rules"].lower() == "true":
        print(f"""Data Quality engine is ENABLED for table {config["table_name"]}. Building rules...""")
        dq = Dq(spark, config)
        dq.run_dq_rules(df)
    else:
        print(f"""Data Quality engine is DISABLED for table {config["table_name"]}.""")
