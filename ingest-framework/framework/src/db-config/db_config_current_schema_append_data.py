...
current_db_config_table = "dev.config.non_cdc_confg_tmp"
...
spark.createDataFrame(data).writeTo(f"{new_db_config_table}").createOrReplace()

# COMMAND ----------

spark.sql(f"select * from {new_db_config_table} where is_table_enabled = 'True'").display()
